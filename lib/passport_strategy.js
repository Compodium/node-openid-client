...
OpenIDConnectStrategy.prototype.authenticate = function authenticate(req, options) {
  (async () => {
    ...
    if (Object.keys(reqParams).length === 0) {
      ...
      req.session[sessionKey] = pick(params, 'nonce', 'state', 'max_age', 'response_type', 'customer');
    ...
    const {
      state, nonce, max_age: maxAge, code_verifier: codeVerifier, response_type: responseType, customer,
    } = session;
