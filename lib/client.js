...
function authorizationParams(params) {
	...
	Object.entries(authParams).forEach(([key, value]) => {
		...
		} else if (key === "login_hint" && Array.isArray(value)) {
			authParams[key] = value;
  ...
  <form method="post" action="${this.issuer.authorization_endpoint}">
    ${formInputs}
		...
		async decryptJWE(jwe, expectedAlg, expectedEnc = "A128CBC-HS256") {
			...
			if (expectedAlg.match(/^(?:RSA|ECDH)/)) {
				...
				keyOrStore = await this.joseSecret(
					expectedAlg === "dir" ? expectedEnc : expectedAlg
				);
			}
		...
		async validateIdToken(tokenSet, nonce, returnedBy, maxAge, state) {
			...
			const timestamp = now();
			const { protected: header, payload, key } = await this.validateJWT(
				idToken,
				expectedAlg
			);
		...
		async validateJWT(
			...
			if (payload.azp !== undefined) {
				let { additionalAuthorizedParties } =
		...
		async userinfo(
			...
			if (options.method !== "GET" && options.method !== "POST") {
				throw new TypeError(
					"#userinfo() method can only be POST or a GET"
				);
			}
			...
			if (jwt) {
				...
				if (!this.userinfo_signed_response_alg) {
					...
					({ payload: parsed } = await this.validateJWTUserinfo(
						userinfo
					));
				}
			} else {
		...
		async fetchDistributedClaims(claims, tokens = {}) {
			...
			await Promise.all(
				distributedSources.map(async ([sourceName, def]) => {
					try {
						const requestOpts = {
							headers: {
								Accept: "application/jwt",
								Authorization: authorizationHeaderValue(
									def.access_token || tokens[sourceName]
								),
							},
						};
		...
		async requestObject(
			requestObject = {},
			{
				sign: signingAlgorithm = this.request_object_signing_alg ||
					"none",
				encrypt: {
					alg: eKeyManagement = this.request_object_encryption_alg,
					enc: eContentEncryption = this
			...
			if (signingAlgorithm === "none") {
				...
				if (symmetric) {
					...
					if (!key) {
						throw new TypeError(
							`no key to sign with found for alg ${signingAlgorithm}`
						);
					}
				}
...
function dpopProof(payload, jwk) {
	...
	try {
		...
		throw new TypeError(
			'"DPoP" option must be an asymmetric private key to sign the DPoP Proof JWT with'
		);
	}
	...
	if (!alg && this.issuer.dpop_signing_alg_values_supported) {
		...
		alg = this.issuer.dpop_signing_alg_values_supported.find((a) =>
			algs.has(a)
		);
	}
